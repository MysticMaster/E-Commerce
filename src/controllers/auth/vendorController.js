...
const postVerifyOTP = async (req, res) => {
    try {
        ...
        if(type === 'login'){
            ...
            const token = await createToken(vendor, maxAge);
            if (fcmToken) {
        ...
        if(type === 'signup'){
            ...
            await Vendor.create({userId: user.userId, vendorName: generateRandomString(10)});

            return res.status(201).json(HTTPResponse(201, {}, 'Signup Successful'));
        }
